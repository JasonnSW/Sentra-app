...
export async function getTransactions(): Promise<any> {
  ...
  const res = await apiClient<any>("/api/v1/budget/transactions", "GET", {
    headers: {
...
export async function getTransactionsById(id: string): Promise<any> {
  const token = await getAccessToken();
  const res = await apiClient<any>(`/api/v1/budget/transactions/${id}`, "GET", {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export async function getTransactionsByPeriodWeek(Week: string): Promise<any> {
  const token = await getAccessToken();
...
export async function updateTransaction(): Promise<any> {
  const token = await getAccessToken();
  const res = await apiClient<any>(`/api/v1/budget/transactions`, "PUT", {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export async function createTransaction(body: any): Promise<any> {
  const token = await getAccessToken();
  const res = await apiClient<any>(`/api/v1/budget/transactions`, "POST", {
    headers: {
      Authorization: `Bearer ${token}`,
    },
    body: body,
