...
import { DATE_OPTIONS } from "../data/home";
import useDateRange from "../hooks/use-date-selector";
  ...
  return (
    <View className="flex-row items-center justify-between rounded-lg">
      <>
        <View className="flex-row items-center gap-x-2 flex-1">
          ...
          {/* {open && (
            <DateTimePicker
              ...
              onChange={(event : any, date : any) => onChangeDate(event, date)}
            />
          )} */}
        ...
        <TouchableOpacity
          onPress={() => router.replace("/(sentra-pay)/sentra-pay")}
        >
          <Text className="text-white ml-2 text-sm">Lihat selengkapnya...</Text>
        </TouchableOpacity>
      </>
